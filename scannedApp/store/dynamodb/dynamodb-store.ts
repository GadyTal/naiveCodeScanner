...
export class DynamoDbStore implements ProductStore {
  ...
  // name"}],"ComparisonOperator": "GT","AttributeValueList":[{"S":*

  public getProductByNameWithInjection = async (
    name: string
  ...
  public getProduct = async (id: string): Promise<Product | undefined> => {
    ...
    const restult: GetCommandOutput = await DynamoDbStore.ddbDocClient.send(
      params
    );
